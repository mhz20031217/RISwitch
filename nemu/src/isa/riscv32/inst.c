...
#define BCOND(cond) if ((cond)) s->dnpc = s->pc + imm
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, int64_t result = ((int64_t) src1 * (int64_t) src2); R(rd) = (int32_t) (result >> 32));
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, if (src2 == 0) R(rd) = ~0U; else R(rd) = (int64_t) src1 / (int64_t) src2);
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, if (src2 == 0) R(rd) = ~0U; else R(rd) = R(rd) = src1 / src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, if (src2 == 0) R(rd) = 0U; else R(rd) = R(rd) = (int64_t) src1 % (int64_t) src2);
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, if (src2 == 0) R(rd) = src1; else R(rd) = R(rd) = src1 % src2);
