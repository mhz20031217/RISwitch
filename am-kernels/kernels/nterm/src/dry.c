 ...
 *              - sh_printf, scanf (outside the measurement loop)
 ...
 *              "sh_printf"s are provided marking the points "Start Timer"
  ...
  /* Main and Proc_0 in the Ada version             */
{
 ...
 Number_Of_Runs = NUMBER_OF_RUNS;
  ...
  sh_printf ("Dhrystone Benchmark, Version %s\n", Version);

  Done = false;
  while (!Done) {

    sh_printf ("Trying %d runs through Dhrystone.\n", Number_Of_Runs);
  ...
  if (!check(Int_Glob == 5)) {
    sh_printf("Int_Glob:            %d\n", Int_Glob);
    sh_printf("        should be:   %d\n", 5);
  }
  if (!check(Bool_Glob == 1)) {
    sh_printf("Bool_Glob:           %d\n", Bool_Glob);
    sh_printf("        should be:   %d\n", 1);
  }
  if (!check(Ch_1_Glob == 'A')) {
    sh_printf("Ch_1_Glob:           %c\n", Ch_1_Glob);
    sh_printf("        should be:   %c\n", 'A');
  }
  if (!check(Ch_2_Glob == 'B')) {
    sh_printf("Ch_2_Glob:           %c\n", Ch_2_Glob);
    sh_printf("        should be:   %c\n", 'B');
  }
  if (!check(Arr_1_Glob[8] == 7)) {
    sh_printf("Arr_1_Glob[8]:       %d\n", Arr_1_Glob[8]);
    sh_printf("        should be:   %d\n", 7);
  }
  if (!check(Arr_2_Glob[8][7] == Number_Of_Runs + 10)) {
    sh_printf("Arr_2_Glob[8][7]:    %d\n", Arr_2_Glob[8][7]);
    sh_printf("        should be:   Number_Of_Runs + 10\n");
  }

  if (!check((int)Ptr_Glob->Discr == 0)) {
    sh_printf("Ptr_Glob->Discr:             %d\n", Ptr_Glob->Discr);
    sh_printf("        should be:   %d\n", 0);
  }
  if (!check(Ptr_Glob->variant.var_1.Enum_Comp == 2)) {
    sh_printf("Ptr_Glob->Enum_Comp:         %d\n", Ptr_Glob->variant.var_1.Enum_Comp);
    sh_printf("        should be:   %d\n", 2);
  }
  if (!check(Ptr_Glob->variant.var_1.Int_Comp == 17)) {
    sh_printf("Ptr_Glob->Int_Comp:          %d\n", Ptr_Glob->variant.var_1.Int_Comp);
    sh_printf("        should be:   %d\n", 17);
  }
  if (!check(strcmp(Ptr_Glob->variant.var_1.Str_Comp, "DHRYSTONE PROGRAM, SOME STRING") == 0)) {
    sh_printf("Ptr_Glob->Str_Comp:          %s\n", Ptr_Glob->variant.var_1.Str_Comp);
    sh_printf("        should be:   DHRYSTONE PROGRAM, SOME STRING\n");
  }

  if (!check((int)Next_Ptr_Glob->Discr == 0)) {
    sh_printf("Next_Ptr_Glob->Discr:             %d\n", Next_Ptr_Glob->Discr);
    sh_printf("        should be:   %d\n", 0);
  }
  if (!check(Next_Ptr_Glob->variant.var_1.Enum_Comp == 1)) {
    sh_printf("Next_Ptr_Glob->Enum_Comp:         %d\n", Next_Ptr_Glob->variant.var_1.Enum_Comp);
    sh_printf("        should be:   %d\n", 1);
  }
  if (!check(Next_Ptr_Glob->variant.var_1.Int_Comp == 18)) {
    sh_printf("Next_Ptr_Glob->Int_Comp:          %d\n", Next_Ptr_Glob->variant.var_1.Int_Comp);
    sh_printf("        should be:   %d\n", 18);
  }
  if (!check(strcmp(Next_Ptr_Glob->variant.var_1.Str_Comp, "DHRYSTONE PROGRAM, SOME STRING") == 0)) {
    sh_printf("Next_Ptr_Glob->Str_Comp:          %s\n", Next_Ptr_Glob->variant.var_1.Str_Comp);
    sh_printf("        should be:   DHRYSTONE PROGRAM, SOME STRING\n");
  }

  if (!check(Int_1_Loc == 5)) {
    sh_printf("Int_1_Loc:           %d\n", Int_1_Loc);
    sh_printf("        should be:   %d\n", 5);
  }
  if (!check(Int_2_Loc == 13)) {
    sh_printf("Int_2_Loc:           %d\n", Int_2_Loc);
    sh_printf("        should be:   %d\n", 13);
  }
  if (!check(Int_3_Loc == 7)) {
    sh_printf("Int_3_Loc:           %d\n", Int_3_Loc);
    sh_printf("        should be:   %d\n", 7);
  }
  if (!check(Enum_Loc == 1)) {
    sh_printf("Enum_Loc:            %d\n", Enum_Loc);
    sh_printf("        should be:   %d\n", 1);
  }

  if (!check(strcmp(Str_1_Loc, "DHRYSTONE PROGRAM, 1'ST STRING") == 0)) {
    sh_printf("Str_1_Loc:           %s\n", Str_1_Loc);
    sh_printf("        should be:   DHRYSTONE PROGRAM, 1'ST STRING\n");
  }
  if (!check(strcmp(Str_2_Loc, "DHRYSTONE PROGRAM, 2'ND STRING") == 0)) {
    sh_printf("Str_2_Loc:           %s\n", Str_2_Loc);
    sh_printf("        should be:   DHRYSTONE PROGRAM, 2'ND STRING\n");
  }

  sh_printf ("Finished in %d ms\n", (int)User_Time);
  sh_printf("==================================================\n");
  sh_printf("Dhrystone %s         %d Marks\n", pass ? "PASS" : "FAIL",
      880900 / (int)User_Time * NUMBER_OF_RUNS/ 500000);
  sh_printf("                   vs. 100000 Marks (i7-7700K @ 4.20GHz)\n");
