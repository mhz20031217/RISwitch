...
3. MMIO 区域
    ...
    UPD 2023.12.20: 还有 VGA Framebuffer 必须 2 字节访存，且必须对齐。
    ...
    9. VGA 显示模式寄存器：大小 4B，地址空间 0x80e00000 ~ 0x80e00004，只写，通过写入 0 值，设置输出模式为字符显存，写入 1，设置输出模式为 VGA。
    10. VGA Framebuffer：大小 512 * 640 * 2B，地址空间 0x80800000 ~ 0x808a0000，只写，对应一个 640x480 的屏幕（行数向上取整到 512），每个 2B 代表 16 位颜色 {4'b0, red[3:0], green[3:0], blue[3:0]}。只有低 12 位有效。
